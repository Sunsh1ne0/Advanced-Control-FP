...
def safe(state, speed):
    ...
    # speed = speed * coef if abs(state[1]) < 22 else move(0)
    if abs(state[1]) < 22:
        speed * coef
    else:
        move(0)
...
def move(position):
    ...
    pend.set_speed(0)
    #
    # pid = PID((50, 5, 0, 0), 0.2)
    # target = np.array((np.pi, 0, 0, 0))
    # error, state = pend.get_state("float")
    while abs(abs(state[0]) - np.pi) > 0.1 or abs(state[2]) > 0.005:
        ...
        if error:
            ...
            continue
    #
    #     speed = pid.step_unsafe(state, target)
    #     # speed = safe(state, speed)
    #
        # pend.set_speed(np.sign(state[2])*120)
        hold_state(state, 0)
...
def main():
    ...
    while True:
        ...
        # speed = ener.step(state, target)
        # speed = swing.step(state, 200)
